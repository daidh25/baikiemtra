...
public static class Validator
{
    ...
    public static string KiemTraNhapSoDienThoai(string fieldName)
    {
        string result;
        while (true)
        {
            ...
            if (Regex.IsMatch(result, @"^[0-9]{10,11}$"))
            {
                break;
            }
            Console.WriteLine($"Vui lòng nhập {fieldName} hợp lệ.");
    ...
    public static string KiemTraNhapEmail(string fieldName)
    {
        string result;
        while (true)
        {
            Console.WriteLine($"Nhập {fieldName}:");
            result = Console.ReadLine();
            if (Regex.IsMatch(result, @"^[\w-]+(\.[\w-]+)*@([\w-]+\.)+[a-zA-Z]{2,20}$"))
            {
                break;
            }
            Console.WriteLine($"Vui lòng nhập {fieldName} hợp lệ.");
    ...
    {
        if (Regex.IsMatch(input, @"[!@#$%^&*()_+=\[{\]};:<>|./,-]") && !Regex.IsMatch(input, @"[@]"))
        {
            return false;
        }
